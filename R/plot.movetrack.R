...
#' @param id id of animal to plot.  If NULL - use subset or first animal. 
#' @param subset what portion of data to plot.  If null, use "id" or first animal.
#' @param ... options to be passed to plot functions
#' @S3method plot movetrack

plot.movetrack <- function(movetrack=NULL, id = NULL, subset = NULL, time, x, y=NULL, layout = NULL, auto.par = NULL,  ...)
{
 if(is.null(subset))
  if(is.null(id)) subset <- movetrack$id == movetrack$id[1] else
    subset <- movetrack$id == id
  
 layout(cbind(1:1,2:3))
 par(mar = c(0,4,0,0), oma = c(4,0,4,4), xpd = NA)
 with(subset(movetrack, subset), {
